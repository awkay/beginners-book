...
(def david {:person/name   "Daffyd"
            :person/sector {:sector/job "programmer"}
            :person/pet    {:pet/cat "Monty"
                            :pet/dog "Yugi"}})
(get-in david [:person/pet :pet/dog])
(get-in david [:person/sector :sector/job])


(get-in david [[:person/pet :pet/cat] [:person/sector :sector/job]])
;; Can I only pull values from one section, and not multiple in one line? The above only gives nil
...
(def joe {:person/name    "Joe"
          :person/address {:address/street "123 main"}})
(def joe2 (assoc-in joe [:person/address :address/city] "New York"))
joe2
...
(conj #{1 2 3} 4)
;; sets are unordered, so using 'conj' the new element can appear at any position within the set
(disj #{1 2 3} 2)
(contains? #{1 2 3} 1)
(contains? #{1 2 3} 0)
...
(set/union #{:a :d} #{:a :b :c})
;;#{:c :b :d :a}
;; union function combines the elements from both sets into a new set
;; it ensures each element appears only once
;; the set is unordered
(set/difference #{:a :d} #{:a :b :c})
;;#{:d}
;; difference function computes the set of elements that are in the first set but NOT the second
(set/intersection #{:a :c :d} #{:a :b :c})
;;#{:c :a}
;;intersection function computes the set of elements present in both sets
(set/subset? #{:a :b :c} #{:a})
;;false
;; subset? expression checks if the set #{a} is a subset of the superset #{:a :b :c}
;; the superset #{:a :b :c} contains elements :b and :c that are not present in the subset #{:a}
(set/subset? #{:a} #{:a :b :c})
;; true
;; the superset #{:a} is contained in the subset #{:a :b :c}
(set/superset? #{:a} #{:a :b :c})
;; false
;; superset checks if the set #{:a :b :c} is a superset of the set #{:a}
...
;; both seeking:
(set/intersection #{:dating :friends} #{:long-term :friends :hook-ups})
;; #{:friends}
...
;; No. assoc replaces the value specified, get identifies the value specified
